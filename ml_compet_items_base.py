...
# CONFIG FICHIERS
# =========================
ITEMS_FILE = "kaggle_data/items.csv"          # <-- fichier de base
...
def load_data():
    ...
    # on met toutes les colonnes en lowercase pour être robustes
    items_df.columns = [c.lower() for c in items_df.columns]
...
def train_test_split_loo(interactions):
    """
    Leave-One-Out par utilisateur :
      - on trie par temps
      - on garde la dernière interaction de chaque user pour le test
      - le reste pour le train
    """
    df = interactions.sort_values(["u", "t"]).copy()
    df["rank"] = df.groupby("u")["t"].rank(method="first", ascending=True)

    last_rank = df.groupby("u")["rank"].transform("max")

    test = df[df["rank"] == last_rank].copy()
    train = df[df["rank"] != last_rank].copy()
...
def make_soup_df(items_df, item_to_idx):
    """
    items_df vient de kaggle_data/items.csv
    On suppose qu'il y a au moins :
      - 'i' : ID du livre
      - 'title' ou 'titre'
      - 'authors' ou 'author'
      - 'subjects' ou 'subject'
      - éventuellement 'publisher'
    """

    # Ne garder que les items qui apparaissent effectivement dans le train
    items_df = items_df[items_df["i"].isin(item_to_idx.keys())].copy()

    # S'assurer que l'ordre = colonnes de R (via col_idx)
    ...
    def build_soup(row):
        title = get_col(row, "title", "titre")
        authors = get_col(row, "authors", "author", "auteurs")
        subjects = get_col(row, "subjects", "subject")
        publisher = get_col(row, "publisher", "publishe", "editeur")

        # on pondère un peu plus les subjects
        soup = f"{title} {authors} {publisher} "
        soup += (subjects + " ") * 3
        return soup.strip()
...
# =========================
# 6. ÉVALUATION MAP@10 (LOO)
# =========================

def map_at_10_loo(test_df, R_train, sim_cf, sim_content, user_to_idx, idx_to_item, alpha):
    """
    MAP@10 avec :
      - R_train : matrice interactions (train)
      - sim_cf : matrice item-item CF
      - sim_content : matrice item-item contenu
      - alpha : poids CF, (1 - alpha) poids contenu
    """
    print(f"\n🐱 Évaluation LOO hybride (alpha CF = {alpha:.2f}, contenu = {1-alpha:.2f})")

    # ground truth {user_id: item_emprunté_dans_le_test}
    ...
    for u in tqdm(users_eval, desc="Users", leave=False):
        ...
        # scores item-item pour cet user
        ...
        # top K indices dans l'espace items
        ...
        true_items = gt[u]  # set avec un seul item (Leave-One-Out)
...
def main():
    ...
    parser.add_argument("--evaluate", action="store_true",
                        help="Split Leave-One-Out + MAP@10")
    parser.add_argument("--submission", action="store_true",
                        help="Train full + submission.csv")
    parser.add_argument("--alpha", type=float, default=0.5,
                        help="Poids CF pour le mode submission")
    ...
    if args.evaluate:
        # Split Leave-One-Out
        train_df, test_df = train_test_split_loo(interactions)

        # Matrice R sur TRAIN
        R_train, user_to_idx, item_to_idx, idx_to_user, idx_to_item = build_interaction_matrix(train_df)

        # Contenu (TF-IDF) aligné sur items du train
        ...
        for alpha in [0.0, 0.25, 0.5, 0.75, 1.0]:
            score = map_at_10_loo(test_df, R_train, sim_cf, sim_content, user_to_idx, idx_to_item, alpha)
            print(f"✅ MAP@10 (LOO, alpha={alpha}) = {score:.5f}")
            if score > best_map:
                best_map = score
                best_alpha = alpha
        ...
        # Optionnel : entraîner sur toutes les interactions avec ce best_alpha pour produire une submission direct
        ...
        # Train sur toutes les interactions avec alpha donné
        ...
        print("Utilisation :")
        print("  python3 ml_compet_items_base.py --evaluate")
        print("  python3 ml_compet_items_base.py --submission --alpha 0.5")
